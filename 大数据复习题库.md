...
| **序号** | **操作描述**                                                 | **HiveQL 语句**                                              |
| -------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| (1)      | 创建一个名为 "student" 的表，包含字段 `id(int)`、`name(string)`、`age(int)`，按 `id` 分区。 | `sql CREATE TABLE student (name STRING, age INT) PARTITIONED BY (id INT); ` |
| (2)      | 向 "student" 表中插入一条记录：`(1, "Alice", 20)`。          | `sql INSERT INTO student PARTITION(id=1) VALUES ('Alice', 20); ` **或使用动态分区：** `sql INSERT INTO student PARTITION(id) VALUES (1, 'Alice', 20); ` |
| (3)      | 查询年龄大于 18 岁的学生信息。                               | `sql SELECT * FROM student WHERE age > 18; `                 |
| (4)      | 计算学生平均年龄。                                           | `sql SELECT AVG(age) FROM student; `                         |
| (5)      | 删除 "student" 表。                                          | `sql DROP TABLE student; `                                   |
...
| **操作描述**                                                 | **HiveQL 语句**                                              |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| 查询所有学生的姓名（name）和分数（score），并按照分数降序排列。 | `sql SELECT name, score FROM student ORDER BY score DESC; `  |
| 查询每个班级（class）的**平均分数**，并将结果命名为 `avg_score`。 | `sql SELECT class, AVG(score) AS avg_score FROM student GROUP BY class; ` |
| 查询分数**大于 80 分**的学生总人数。                         | `sql SELECT COUNT(*) FROM student WHERE score > 80; `        |
| 修改表 `student` 的名称为 `student_info`。                   | `sql ALTER TABLE student RENAME TO student_info; `           |
...
| **操作描述**                                      | **HiveQL 语句**                                              |
| ------------------------------------------------- | ------------------------------------------------------------ |
| 创建表 `employee`                                 | `sql CREATE TABLE employee ( id INT, name STRING, dept STRING, salary INT ); ` |
| 向表中插入一条记录：`(1, '张三', '技术部', 8000)` | `sql INSERT INTO employee VALUES (1, '张三', '技术部', 8000); ` |
| 查询每个部门的平均工资                            | `sql SELECT dept, AVG(salary) AS avg_salary FROM employee GROUP BY dept; ` |
| 查询工资高于 5000 的员工姓名和部门                | `sql SELECT name, dept FROM employee WHERE salary > 5000; `  |
...
| **步骤**          | **HiveQL 语句**                                              | **备注**                                                     |
| ----------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **创建表**        | `sql CREATE TABLE users ( uid INT, uname STRING, udept STRING ) ROW FORMAT DELIMITED FIELDS TERMINATED BY ','; ` | `ROW FORMAT DELIMITED` 指定了数据的分隔符。                  |
| **加载本地文件**  | `sql LOAD DATA LOCAL INPATH '/tmp/user_data.txt' INTO TABLE users; ` | `LOCAL` 表示文件在 Hive 客户端所在的本地文件系统。若无 `LOCAL`，则表示文件在 HDFS 上。 |
| **GROUP BY 查询** | `sql SELECT udept, COUNT(uid) AS dept_count FROM users GROUP BY udept HAVING COUNT(uid) > 1; ` | 统计人数大于 1 的部门及其人数。                              |
...
5. 在IDEA中开发Hadoop程序需要添加的依赖包括（**ABCE**）
...
**注意**: 实际开发中,通常只需要添加`hadoop-client`即可,它已经包含了A、B、C的依赖。
