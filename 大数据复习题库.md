...
11. Hadoop 集群中，通常用于资源调度的组件是 ***\*YARN\****。
12. 在 MapReduce 编程中，用户自定义的 Mapper 类需要继承自 org.apache.hadoop.mapreduce.Mapper，其输出的 Key 和 Value 必须是 Hadoop 自定义的 ***\*Writable\**** 类型。
13. 要使用 Java API 将本地文件 /home/data/input.txt 上传到 HDFS 的 /user/test 目录下，需要使用 FileSystem 类的 ***\*copyFromLocalFile\**** 方法。
14. 在 Hive 中，如果一张表的元数据被删除，但 HDFS 上的数据文件仍然保留，那么这张表是 ***\*外部表（EXTERNAL TABLE）\****。
15. HBase 的数据存储结构是 ***\*表（Table）\****，它由 ***\*行键（Row Key）、列族（Column Family） 和 时间戳（Timestamp）\**** 唯一确定一个单元格（Cell）。
...
2假设你需要在一台Linux服务器上搭建一个伪分布式Hadoop集群（版本3.3.4），请写出以下步骤的关键命令或操作：
...
3 使用HDFS Shell命令完成以下任务：

（1）在HDFS根目录下创建名为"exam"的目录；

（2）将本地当前目录下的"data.txt"文件上传到HDFS的"/exam"目录；

（3）查看"/exam/data.txt"文件的内容；

（4）统计"/exam"目录下所有文件的总大小（以字节为单位）；
...
1 已知Hive已安装并启动，Metastore使用Derby数据库。请写出以下操作的HQL语句：

（1）创建一个名为"student"的表，包含字段id(int)、name(string)、age(int)，按id分区；

（2）向"student"表中插入一条记录：(1, "Alice", 20)；
...
2 假设您在 Hive 中已经有一个名为 student 的表，包含以下字段：id(INT), name(STRING), score(INT), class(STRING)。
...
使用Java API编写一段简单代码，实现将本地路径“/home/user/data.txt”的文件上传到HDFS的“/user/hadoop/input”目录下（需写出核心代码及关键注释）。
...
请补全以下WordCount程序的Mapper和Reducer代码框架：
 ...
 编写一个简单的 MapReduce WordCount 示例伪代码
