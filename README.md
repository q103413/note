...
宝塔域名迁移到目标服务器 11.22.33.44，配置好Nginx、CDN，迁移资源文件。

迁移两类域名，一类域名访问落地页，另一类域名导航页重定向到落地页。

申请测试二级域名（添加ns解析 migration.xxx.com migration.xxx.com cname xxx.vip.）

## 二、准备

* Nginx配置文件目录 /usr/local/nginx1.23/conf/vhosts
* Nginx资源文件目录 /data/front/pro-daohang
...
进阶：通过CDN添加端口访问，好处是以后方便添加域名（不用修改NGINX配置文件）&#x20;

本质上是域名对应端口 miss\_navigation\_port 26115&#x20;
...
// Some code
cd C:/work/xxx.zip 
scp -P 22 xxx.zip root@11.22.33.44:/data/front/
```
...
scp传文件指定端口 在Linux中, 两台主机传送文件的时候, 经常用到scp命令. 通常情况下, ssh的默认端口都会自定义
